...
class CollectionSignViewSet(api_base.ViewSet):
    ...
    def sign(self, request, *args, **kwargs):
        ...
        content_units = self.get_content_units_to_sign(request, repository)

        sign_task = call_sign_task(
            signing_service,
            repository,
            content_units,
        )
        ...
        return Response(
            data={"task_id": sign_task.pk},
            status=status.HTTP_202_ACCEPTED
        )

    def get_content_units_to_sign(self, request, repository):
        """
        Returns a list of content units to sign.
    ...
    def get_repository(self, request):
        ...
        """
        try:
            return AnsibleDistribution.objects.get(
                base_path=request.data["repository"]
            ).repository
        except KeyError:
            raise ValidationError(
                _("repository field is required.")
            )
        except ObjectDoesNotExist:
            raise ValidationError(
                _("Repository %s does not exist.") % request.data["repository"]
            )

    def get_signing_service(self, request):
        try:
            return SigningService.objects.get(
                name=request.data['signing_service']
            )
        except KeyError:
            raise ValidationError(
                _('signing_service field is required.')
            )
        except ObjectDoesNotExist:
            raise ValidationError(
