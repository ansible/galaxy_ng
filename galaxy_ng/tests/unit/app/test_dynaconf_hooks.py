...
def test_dynaconf_hooks(do_stuff, extra_settings, expected_results):
    if not do_stuff:
        return
    ...
    # import epdb; epdb.st()
    print(f'XSETTINGS: {id(xsettings)}')

    new_settings = post_hook(xsettings, testing=True, run_dynamic=False, run_validate=False)
