...
class ContainerSyncandManifestListTestCase(TestCaseUsingBindings):
    ...
    - perform sync
    - verify that the repo has two images with tags ml_i and manifest_a, that manifest_a is
      a manifest and that ml_i is a manifest list with two manifests in it
    ...
    def setUp(self):
        ...
        self.addCleanup(
            ...
            f"{self.galaxy_api_prefix}/_ui/v1/"
            f"execution-environments/registries/{self.docker_registry.pk}/")

    def _delete_remote_repo(self, remote):
        self.smash_client.delete(
            f"{self.galaxy_api_prefix}/_ui/v1/execution-environments/repositories/{remote.name}/")
    ...
    def test_manifests_and_remote_sync(self):
        ...
        # Note, the manifest_a tag in the pulp test repo points to a manifest
        # that's part of the ml_i manifest list
        # so it won't show up in here since exclude_child_manifests filters out any manifest
        # that is part of a manifest list.
