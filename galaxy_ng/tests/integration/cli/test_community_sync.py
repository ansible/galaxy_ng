...
def test_community_collection_download_count_sync(ansible_config):
    ...
    while next_url:
        ...
        for collection in ds['results']:
            ...
            check_url = f'/api/v3/plugin/ansible/content/community/collections/index/{namespace}/{name}/'
            try:
    ...
    remote_task = api_client.request(remotes['community']['pulp_href'], method='PATCH', args=community_remote_config)
    ...
    sync_task = api_client.request(repos['community']['pulp_href'] + 'sync/', method='POST', args=sync_payload)
    ...
    check_url = f'/api/v3/plugin/ansible/content/community/collections/index/{sync_collection[0]}/{sync_collection[1]}/'
