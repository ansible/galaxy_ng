...
def test_download_url_redirect(ansible_config, artifact, upload_artifact):
    """Ensure download/<filename> URL redirects to the correct location"""
    config = ansible_config("partner_engineer")
    api_client = get_client(config, request_token=True, require_auth=True)
    ...
    with tempfile.TemporaryDirectory() as dir:
        ...
        filename = f"{artifact.namespace}-{artifact.name}-{artifact.version}.tar.gz"
        tarball_path = f"{dir}/{filename}"
        url = f"{api_root}download/{filename}"
