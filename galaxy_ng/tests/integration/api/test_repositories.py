...
from galaxy_ng.tests.integration.utils.repo_management_utils import create_repo_and_dist, \
    create_test_namespace, upload_new_artifact, add_content_units, search_collection_endpoint, verify_repo_data
...
class TestRepositories:
    ...
    def test_copy_cv_endpoint(self, galaxy_client):
        ...
        artifact = upload_new_artifact(gc_admin, namespace_name, test_repo_name_1, "1.0.1", tags=["application"])
        ...
        expected = [{"cv_name": artifact.name, "repo_name": test_repo_name_1, "is_signed": False},
    ...
    def test_copy_signed_cv_endpoint(self, galaxy_client):
        ...
        copy_content_between_repos(gc_admin, content_units, repo_pulp_href_1, [repo_pulp_href_2])
        matches, results = search_collection_endpoint(gc_admin, name=artifact.name)
        expected = [{"cv_name": artifact.name, "repo_name": test_repo_name_1, "is_signed": True},
