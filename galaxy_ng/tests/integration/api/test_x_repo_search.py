...
class TestXRepoSearch:
    ...
    def test_search_same_collection_diff_versions_same_repo(self, galaxy_client):
        ...
        artifact = upload_new_artifact(gc, namespace_name, test_repo_name, "1.0.1", key, tags=["application"])
        ...
        assert matches == 2
        expected = [{"repo_name": test_repo_name, "cv_version": "1.0.2", "is_highest": True},
    ...
    def test_search_same_collection_diff_versions_diff_repo(self, galaxy_client):
        ...
        namespace_name = create_test_namespace(gc)
        key = generate_random_string()
        artifact_1 = upload_new_artifact(gc, namespace_name, test_repo_name_1, "1.0.1", key, tags=["application"])
        upload_new_artifact(gc, namespace_name, test_repo_name_2, "1.0.2", key, tags=["application"])
        ...
        expected = [{"repo_name": test_repo_name_1, "cv_name": artifact_1.name, "cv_version": "1.0.1"},
    ...
    def test_search_same_collection_diff_repo_same_versions(self, galaxy_client):
        ...
        repo_pulp_href_2 = create_repo_and_dist(gc, test_repo_name_2)
        namespace_name = create_test_namespace(gc)
        artifact = upload_new_artifact(gc, namespace_name, test_repo_name_1, "1.0.1", tags=["application"])
        collection_resp = gc.get(f"pulp/api/v3/content/ansible/collection_versions/?name={artifact.name}")
        ...
        expected = [{"repo_name": test_repo_name_1, "cv_name": artifact.name, "cv_version": "1.0.1"},
    ...
    def test_search_upload_diff_collection_diff_namespaces(self, galaxy_client):
        ...
        artifact_1 = upload_new_artifact(gc, namespace_name, test_repo_name, "1.0.1", tags=["application"])
        artifact_2 = upload_new_artifact(gc, namespace_name_mod, test_repo_name, "1.0.2", tags=["application"])
        ...
        content_units = [collection_resp_1["results"][0]["pulp_href"], collection_resp_2["results"][0]["pulp_href"]]
        add_content_units(gc, content_units, repo_pulp_href)
        _, results = search_collection_endpoint(gc, repository_name=test_repo_name)
        expected = [{"repo_name": test_repo_name, "cv_name": artifact_1.name, "cv_version": "1.0.1"},
    ...
    def test_search_is_highest_changes_after_deletion(self, galaxy_client):
        ...
        _, results = search_collection_endpoint(gc, name=artifact.name)
        expected = [{"cv_version": "1.0.2", "is_highest": True}, {"cv_version": "1.0.1", "is_highest": False}]
        assert verify_repo_data(expected, results)
        delete_collection(gc, namespace_name, artifact.name, version="1.0.2", repository=test_repo_name)
        _, results = search_collection_endpoint(gc, name=artifact.name)
    ...
    def test_search_deprecated_collection(self, galaxy_client):
        ...
        repo_pulp_href = create_repo_and_dist(gc, test_repo_name)
        namespace_name = create_test_namespace(gc)
        artifact = upload_new_artifact(gc, namespace_name, test_repo_name, "1.0.1", tags=["application"])
        collection_resp = gc.get(f"pulp/api/v3/content/ansible/collection_versions/?name={artifact.name}")
        content_units = [collection_resp["results"][0]["pulp_href"]]
        add_content_units(gc, content_units, repo_pulp_href)
    ...
    def test_search_by_repository_name_or_operator(self, galaxy_client):
        ...
        namespace_name = create_test_namespace(gc)
        artifact_1 = upload_new_artifact(gc, namespace_name, test_repo_name_1, "1.0.1", tags=["application"])
        artifact_2 = upload_new_artifact(gc, namespace_name, test_repo_name_1, "1.0.2", tags=["application"])
    ...
    def test_search_by_version(self, galaxy_client):
        ...
        artifact = upload_new_artifact(gc, namespace_name, test_repo_name, version, tags=["application"])
        collection_resp = gc.get(f"pulp/api/v3/content/ansible/collection_versions/?name={artifact.name}")
        content_units = [collection_resp["results"][0]["pulp_href"]]
        add_content_units(gc, content_units, repo_pulp_href)
    ...
    def test_search_is_highest_true_false(self, galaxy_client, is_highest, cv_version):
        ...
        artifact_1 = upload_new_artifact(gc, namespace_name, test_repo_name, "4.0.1", key, tags=["application"])
        upload_new_artifact(gc, namespace_name, test_repo_name, "4.0.2", key, tags=["application"])

        collection_resp = gc.get(f"pulp/api/v3/content/ansible/collection_versions/?name={artifact_1.name}")
        content_units = [collection_resp["results"][0]["pulp_href"], collection_resp["results"][1]["pulp_href"]]
        add_content_units(gc, content_units, repo_pulp_href)
        matches, results = search_collection_endpoint(gc, repository_name=test_repo_name, is_highest=is_highest)
        expected = [{"repo_name": test_repo_name, "cv_name": artifact_1.name, "cv_version": cv_version}]
        assert verify_repo_data(expected, results)
        assert matches == 1
    ...
    def test_search_by_is_deprecated_true_false(self, galaxy_client, is_deprecated):
        ...
        artifact_1 = upload_new_artifact(gc, namespace_name, test_repo_name, "1.0.1", tags=["application"])
        artifact_2 = upload_new_artifact(gc, namespace_name, test_repo_name, "4.0.1", tags=["application"])
        ...
        matches, results = search_collection_endpoint(gc, is_deprecated=is_deprecated, repository_name=test_repo_name)
    ...
    def test_search_by_tags(self, galaxy_client):
        ...
        artifact_2 = upload_new_artifact(gc, namespace_name, test_repo_name, "4.0.2", tags=["application"])
        ...
        matches, results = search_collection_endpoint(gc, tags="application,test_tag_2", repository_name=test_repo_name)
    ...
    def test_search_by_q(self, galaxy_client):
        ...
        artifact_1 = upload_new_artifact(gc, namespace_name, test_repo_name, "1.0.1", key, tags=["application"])
        artifact_2 = upload_new_artifact(gc, namespace_name, test_repo_name, "4.0.2", tags=["application"])
    ...
    def test_search_by_is_signed_true_false(self, galaxy_client, is_signed):
        ...
        matches, results = search_collection_endpoint(gc, is_signed=is_signed, repository_name=test_repo_name)
    ...
    def test_search_by_distribution_id(self, galaxy_client):
        ...
        repo_pulp_href = create_repo_and_dist(gc, test_repo_name)
        namespace_name = create_test_namespace(gc)
        artifact = upload_new_artifact(gc, namespace_name, test_repo_name, "1.0.0", tags=["application"])
        collection_resp = gc.get(f"pulp/api/v3/content/ansible/collection_versions/?name={artifact.name}")
        content_units = [collection_resp["results"][0]["pulp_href"]]
        add_content_units(gc, content_units, repo_pulp_href)
    ...
    def test_search_by_dependency(self, galaxy_client):
        ...
        artifact = upload_new_artifact(gc, namespace_name, test_repo_name, "1.0.0", dependencies={dep_name: "1.0.0"},
                                       tags=["application"])
        collection_resp = gc.get(f"pulp/api/v3/content/ansible/collection_versions/?name={artifact.name}")
        content_units = [collection_resp["results"][0]["pulp_href"]]
        add_content_units(gc, content_units, repo_pulp_href)
    ...
    def test_search_version_range(self, galaxy_client):
        ...
        repo_pulp_href = create_repo_and_dist(gc, test_repo_name)
        namespace_name = create_test_namespace(gc)
        artifact_1 = upload_new_artifact(gc, namespace_name, test_repo_name, "6.6.6", tags=["application"])
        collection_resp_1 = gc.get(f"pulp/api/v3/content/ansible/collection_versions/?name={artifact_1.name}")

        artifact_2 = upload_new_artifact(gc, namespace_name, test_repo_name, "8.8.8", tags=["application"])
        collection_resp_2 = gc.get(f"pulp/api/v3/content/ansible/collection_versions/?name={artifact_2.name}")

        artifact_3 = upload_new_artifact(gc, namespace_name, test_repo_name, "12.6.6", tags=["application"])
        ...
        matches, results = search_collection_endpoint(gc, version_range='>=6.6.6,<8.8.8',
        ...
        expected = [{"cv_name": artifact_2.name, "repo_name": test_repo_name},
    ...
    def test_search_non_existing_repo(self, galaxy_client):
        ...
        repo_pulp_href_1 = create_repo_and_dist(gc, test_repo_name_1)
        namespace_name = create_test_namespace(gc)
        artifact_1v1 = upload_new_artifact(gc, namespace_name, test_repo_name_1, "1.0.1", tags=["application"])
    ...
    def test_search_after_deletion(self, galaxy_client):
        ...
        namespace_name = create_test_namespace(gc)
        artifact_3v1 = upload_new_artifact(gc, namespace_name, test_repo_name_2, "1.0.1", tags=["application"])
        ...
        delete_collection(gc, namespace_name, artifact_3v1.name, version="1.0.1", repository=test_repo_name_3)
    ...
    def test_is_highest_per_repo(self, galaxy_client):
        ...
        artifact_2v2 = upload_new_artifact(gc, namespace_name, test_repo_name_1, "1.0.2", key_2, tags=["application"])
        artifact_2v1 = upload_new_artifact(gc, namespace_name, test_repo_name_2, "1.0.1", key_2, tags=["application"])
        ...
        expected = [
            {"repo_name": test_repo_name_1, "cv_name": artifact_2v1.name, "cv_version": "1.0.2", "is_highest": True},
            {"repo_name": test_repo_name_2, "cv_name": artifact_2v1.name, "cv_version": "1.0.1", "is_highest": True}]
        assert verify_repo_data(expected, results)
