...
if [[ -z $HUB_LOCAL ]]; then
    $VENVPATH/bin/pytest --capture=no -m "not standalone_only and not community_only and not rbac_roles and not sync" $@ -v galaxy_ng/tests/integration
    RC=$?
else
    $VENVPATH/bin/pytest --capture=no -m "not cloud_only and not community_only and not rbac_roles and not sync" -v $@ galaxy_ng/tests/integration
    RC=$?
