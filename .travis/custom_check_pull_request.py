...
ISSUE_LABEL_REGEX = re.compile(
    r"^\s*({}):\s+#(\d+)\s*$".format("|".join(ISSUE_LABELS)), re.MULTILINE,
)
...
def check_changelog_record(issue):
    ...
    if reconrds_count == 0:
        LOG.error(f"Missing change log entry for issue #{issue}.")
        return False
    if reconrds_count == 1:
        return True
    LOG.error(f"Multiple change log records found for issue #{issue}.")
    return False


def check_issue_exists(repo_slug, issue):
    response = requests.head(f"{GITHUB_URL}/{repo_slug}/issues/{issue}")
    if not response.ok:
        LOG.error(f"Referenced issue #{issue} not found in repository {repo_slug}.")
        return False
    return True


def check_commit(commit_sha, repo_slug):
    ...
    for label, issue in issue_labels:
        if not check_issue_exists(repo_slug, issue):
            ok = False
...
def main():
    ...
    for commit_sha in commit_list:
        ...
        if not check_commit(commit_sha, repo_slug):
            ok = False
